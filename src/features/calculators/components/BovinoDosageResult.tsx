...
import { calculateBovinoDosage } from '@features/calculators/utils/calculations';
...
interface BovinoDosageResultProps {
...
export function BovinoDosageResult({
  product,
  numeroBovinos,
  pesoPromedioKg,
  consumoMsKgDia,
  ...
  diasTratamiento,
}: BovinoDosageResultProps): JSX.Element {
  const theme = useTheme();
  const addItem = useQuotationStore((s) => s.addItem);

  const isUnica = product.tipoCalculo === 'dosis_unica';

  const dosage = useMemo(
    () =>
      calculateBovinoDosage(product, {
        numeroBovinos,
        pesoPromedioKg,
        consumoMsKgDia,
        consumoAguaLitrosDia,
    ...
    [product, numeroBovinos, pesoPromedioKg, consumoMsKgDia, consumoAguaLitrosDia, diasTratamiento],
  );
  ...
  const handleAdd = (): void => {
    addItem({
      ...
      especialidad: 'Ganadería (Bovinos)',
      categoria: 'ganaderia',
      ...
      <View style={styles.resultsRow}>
        ...
        >
          <Text style={[styles.resultLabel, { color: theme.colors.textSecondary }]}>
            {isUnica ? 'Dosis Única' : 'Producto Diario / Animal'}
          </Text>
