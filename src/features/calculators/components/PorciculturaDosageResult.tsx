...
import { calculatePorciculturaDosage } from '@features/calculators/utils/calculations';
...
interface PorciculturaDosageResultProps {
...
export function PorciculturaDosageResult({
  product,
  numeroCerdos,
  pesoKg,
  consumoAlimentoKgDia,
  consumoAguaLitrosDia,
  diasTratamiento,
}: PorciculturaDosageResultProps): JSX.Element {
  const theme = useTheme();
  const addItem = useQuotationStore((s) => s.addItem);

  const isUnica = product.tipoCalculo === 'dosis_unica' || product.tipoCalculo === 'dosis_alimento_unica';

  const dosage = useMemo(
    () =>
      calculatePorciculturaDosage(product, {
        numeroCerdos,
        pesoKg,
        consumoAlimentoKgDia,
        consumoAguaLitrosDia,
    ...
    [product, numeroCerdos, pesoKg, consumoAlimentoKgDia, consumoAguaLitrosDia, diasTratamiento],
  );
      ...
      <View style={styles.resultsRow}>
        ...
        >
          <Text style={[styles.resultLabel, { color: theme.colors.textSecondary }]}>
            {isUnica ? 'Dosis Única' : 'Cantidad Diaria'}
          </Text>
